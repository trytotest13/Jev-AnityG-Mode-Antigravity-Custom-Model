...
# Auto-detect install dir: Antigravity2 is the current build; fall back to legacy 'antigravity'.
# Override with $env:AGY_INSTALL_DIR if needed.
# NOTE: the new "Antigravity IDE" 2.5.x packaging (resources\app\out\main.js, no app.asar)
# is NOT supported by this mod (old Electron app.asar + language_server.exe only).
$candidates = @()
...
$InstallBase = $candidates | Where-Object { Test-Path (Join-Path $_ "resources\app.asar") } | Select-Object -First 1
if (-not $InstallBase) {
    $newIde = "$env:LOCALAPPDATA\Programs\Antigravity IDE"
    if ($env:AGY_INSTALL_DIR -and (Test-Path (Join-Path $env:AGY_INSTALL_DIR "resources\app\out\main.js"))) { $newIde = $env:AGY_INSTALL_DIR }
    if (Test-Path (Join-Path $newIde "resources\app\out\main.js")) {
        Write-Host "[2/7] NOTE: 'Antigravity IDE' (new 2.5.x packaging) found at $newIde" -ForegroundColor Yellow
        Write-Host "   This script is for the OLD Electron build only. Run deploy-ide.ps1" -ForegroundColor Yellow
        Write-Host "   (or install.bat, which picks the right script) for the new IDE." -ForegroundColor Yellow
        exit 1
    }
    Write-Host "[2/7] HATA: app.asar bulunamadi. Denenen konumlar: $($candidates -join ', ')" -ForegroundColor Red
    exit 1
}
Write-Host "   Hedef kurulum: $InstallBase" -ForegroundColor Gray
$AsarPath = Join-Path $InstallBase "resources\app.asar"
...
$LsBinary = Join-Path $InstallBase "resources\bin\language_server.exe"
...
$ExePath = Join-Path $InstallBase "Antigravity.exe"
