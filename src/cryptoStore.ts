...
export function encryptModels(models: ModelWithKey[] | null): ModelWithKey[] {
  ...
  return models.map((model) =>
    model.apiKey && model.apiKey !== 'none' && !model.encrypted
      ? { ...model, apiKey: encryptString(model.apiKey), encrypted: true }
      : model,
  );
}
...
export function decryptModels(models: ModelWithKey[] | null): ModelWithKey[] {
  if (!models || !Array.isArray(models)) return [];
  return models.map((model) =>
    model.encrypted ? { ...model, apiKey: decryptString(model.apiKey as string), encrypted: false } : model,
  );
}
